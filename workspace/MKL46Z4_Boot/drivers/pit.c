...
void Pit_Timer_Interrupt_Enable(Pit_Channel_t channel){
	PIT->CHANNEL[channel].TCTRL &= ~PIT_TCTRL_TIE_MASK;
	PIT->CHANNEL[channel].TCTRL |= PIT_TCTRL_TIE(0b1);
}

void Pit_Timer_Interrupt_Disable(Pit_Channel_t channel){
	PIT->CHANNEL[channel].TCTRL &= ~PIT_TCTRL_TIE_MASK;
}
